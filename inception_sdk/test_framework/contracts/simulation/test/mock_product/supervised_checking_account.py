...
def execution_schedules():
    maintenance_fee_check_day = vault.get_parameter_timeseries(
        name="maintenance_fee_check_day"
    ).latest()
...
@requires(event_type="APPLY_ACCRUED_OVERDRAFT_FEES", parameters=True, balances="1 day live")
@requires(
...
def post_posting_code(postings, effective_date):
    denomination = vault.get_parameter_timeseries(name="denomination").latest()
    overdraft_buffer = vault.get_parameter_timeseries(name="overdraft_buffer").latest()
    overdraft_fee = Decimal(vault.get_parameter_timeseries(name="overdraft_fee").latest())
    overdraft_fee_accrual = vault.get_parameter_timeseries(name="overdraft_fee_accrual").latest()
    internal_overdraft_fee_account = vault.get_parameter_timeseries(
        name="internal_overdraft_fee_account"
    ).latest()
...
def _check_maintenance_fee(vault, effective_date):
    ...
    maintenance_fee = Decimal(
        vault.get_parameter_timeseries(name="maintenance_fee").at(timestamp=effective_date)
    )
    ...
    if maintenance_fee > 0:
        ...
        minimum_daily_balance = vault.get_parameter_timeseries(
            name="minimum_daily_checking_balance"
        ).at(timestamp=effective_date)
        minimum_monthly_deposit = vault.get_parameter_timeseries(name="minimum_monthly_deposit").at(
            timestamp=effective_date
        )
        internal_maintenance_fee_account = vault.get_parameter_timeseries(
            name="internal_maintenance_fee_account"
        ).latest()
